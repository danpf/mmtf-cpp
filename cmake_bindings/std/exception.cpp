...
#include <exception> // std::bad_exception::bad_exception
#include <exception> // std::bad_exception::operator=
#include <exception> // std::bad_exception::what
#include <exception> // std::exception
#include <exception> // std::exception::exception
#include <exception> // std::exception::operator=
#include <exception> // std::exception::what
...
void bind_std_exception(std::function< pybind11::module &(std::string const &namespace_) > &M)
{
	...
	{ // std::exception file:bits/exception.h line:60
		...
		cl.def( pybind11::init( [](){ return new std::exception(); }, [](){ return new PyCallBack_exception(); } ) );
		cl.def( pybind11::init( [](PyCallBack_exception const &o){ return new PyCallBack_exception(o); } ) );
	...
	std::cout << "B5_[int std::uncaught_exceptions()] ";
}
